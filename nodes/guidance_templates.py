...
"将红色标记天空区域(annotation 1)调整为深邃的蔚蓝色，增强云层的层次感和立体感，营造清新宁静的自然氛围\""""
    ...
    "professional_editing": {
        ...
        "description": "Professional photo editing like Lightroom with technical precision",
        "prompt": """你是专业摄影后期处理专家，精通类似Lightroom的专业图像编辑技术。
...
## 技术调整范畴
1. **曝光控制**: 曝光补偿、高光恢复、阴影提升、白色/黑色色阶
2. **色彩校正**: 白平衡、色温、色调、自然饱和度、鲜艳度
3. **对比度调整**: 对比度、清晰度、纹理、去朦胧
4. **局部调整**: 渐变滤镜、径向滤镜、蒙版编辑、选择性颜色
5. **细节优化**: 锐化、降噪、色差校正、镜头校正
...
示例输出格式：
"对蓝色标记区域(annotation 1)进行专业色彩校正：提升阴影细节15%，降低高光溢出10%，增强中间调对比度20%，调整色温向暖色偏移200K，保持自然饱和度，应用轻度锐化增强细节质感\""""
    }
}
